...
from siriushla.as_ap_sofb.ioc_control import AcqControlWidget
...
class MainWindow(SiriusMainWindow):
    def __init__(self, prefix, acc='TB'):
        super().__init__()
        if not prefix.startswith('ca://'):
            prefix = 'ca://' + prefix
        self.prefix = prefix + acc + '-Glob:AP-Orbit:'
        self.acc = acc
        self.setupui()

    def setupui(self):
        self.setWindowModality(Qt.WindowModal)
        self.setWindowTitle("Orbit Measurement System")
    ...
    def _create_central_widget(self):
        ...
        chans = [chan for chan in chans if 'SinglePass' in chan.address]
        ctr = {k: v for k, v in ctr.items() if 'SinglePass' in k}
    ...
    def _create_ioc_controllers(self):
        ...
        docwid.setWindowTitle("Orbit Control")
        sz_pol = QSizePolicy(QSizePolicy.Preferred, QSizePolicy.Preferred)
        sz_pol.setHorizontalStretch(0)
        sz_pol.setVerticalStretch(1)
        ...
        wid = AcqControlWidget(wid2, self.prefix, self.acc)
        vbl.addWidget(wid)
        return docwid
