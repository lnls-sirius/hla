...
class KicksConfigWidget(BaseWidget):
    ...
    def setupui(self):
        ...
        if self.acc == 'SI':
            ...
            hbl.addWidget(wid)
            vbl.addItem(hbl)
            lbl = QLabel('Trigger Delay', gbox)
            wid = self.create_pair(gbox, 'CorrDelay')
            hbl = QHBoxLayout()
            hbl.addWidget(lbl)
            hbl.addWidget(wid)
            vbl.addItem(hbl)
            lbl = QLabel('Enable PSSOFB', gbox)
            wid = self.create_pair_butled(gbox, 'CorrPSSOFBEnbl')
            hbl = QHBoxLayout()
            hbl.addWidget(lbl)
            hbl.addWidget(wid)
            vbl.addItem(hbl)
            lbl = QLabel('Wait PSSOFB', gbox)
            wid = self.create_pair_butled(gbox, 'CorrPSSOFBWait')
            hbl = QHBoxLayout()
            hbl.addWidget(lbl)
            hbl.addWidget(wid)
            vbl.addItem(hbl)
