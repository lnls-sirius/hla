...
class AuxiliaryRFVoltConv(SiriusDialog):
    """Auxiliary Dialog to RF Voltage Convertions VGap <-> FPGA units."""
    ...
    def _setupUi(self):
        ...
        self._lb_vgap = QLabel('V<sub>gap</sub> [kV]: ')
        ...
        self._sb_vgap.setMaximum(100000)
        self._sb_vgap.setDecimals(3)
        ...
        self._sb_raw.setMaximum(100000)
        self._sb_raw.setDecimals(3)
        self._sb_raw.setStyleSheet("min-width:8em; max-width:8em;")
        self._sb_raw.editingFinished.connect(self._update_inputs)
        ...
        layout.addWidget(self._sb_vgap, 3, 0)
        layout.addWidget(self._sb_raw, 3, 2)

    def _update_inputs(self):
        sender = self.sender().objectName()
        if 'vgap' in sender:
            vgap = self._sb_vgap.value()
            raw = self._aux_conv.conv_vgap_2_raw(vgap)
            self._sb_raw.setValue(raw)
        else:
            raw = self._sb_raw.value()
            vgap = self._aux_conv.conv_raw_2_vgap(raw)
            self._sb_vgap.setValue(vgap)
