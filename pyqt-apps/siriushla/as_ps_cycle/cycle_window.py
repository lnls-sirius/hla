...
from qtpy.QtWidgets import QAbstractItemView, QApplication, QGridLayout, \
    QGroupBox, QHBoxLayout, QLabel, QListWidget, QListWidgetItem, \
    QMessageBox, QProgressBar, QPushButton, QSizePolicy as QSzPlcy, \
    QSpacerItem, QVBoxLayout, QWidget
from siriuspy.cycle import get_psnames, get_sections, Timing
...
from ..widgets import PyDMLedMultiConnection as PyDMLedMultiConn, \
    SiriusMainWindow
from ..widgets.dialog import ProgressDialog, PSStatusDialog
from ..widgets.pvnames_tree import PVNameTree
from .tasks import CreateCyclers, Cycle, CycleTrims, PreparePSCurrentZero, \
    PreparePSOpModeCycle, PreparePSOpModeSlowRef, PreparePSParams, \
    PreparePSSOFBMode, PrepareTiming, RestoreTiming, SaveTiming, VerifyPS
...
class CycleWindow(SiriusMainWindow):
    ...
    def __init__(self, parent=None, checked_accs=(), adv_mode=False):
        ...
        self._setup_ui()
        _log.get_logger().addHandler(
            ProgressListHandler(self._update_progress)
        )
    ...
    def _setup_ui(self):
        ...
        # commands
        lb_prep_ti = QLabel(
            "<h4>Prepare Timing</h4>", self, alignment=Qt.AlignCenter
        )
        ti_ch = [
        ...
        lb_prep_ps = QLabel(
            "<h4>Prepare PS</h4>", self, alignment=Qt.AlignCenter
        )
        ...
        lb_rest_ti = QLabel(
            "<h4>Restore Timing</h4>", self, alignment=Qt.AlignCenter
        )
        ...
        layout.addWidget(
            QLabel("<h3>PS Cycle</h3>", self, alignment=Qt.AlignCenter),
            0,
            0,
            1,
            3,
    ...
    def _update_progress(self, text: str, warning=False, error=False):
        ...
        if warning:
            ...
            pal = self.progress_bar.palette()
            pal.setColor(QPalette.Highlight, self.progress_bar.warning_color)
            self.progress_bar.setPalette(pal)
    ...
    def _handle_checked_items_changed(self, item):
        ...
        if (
            not self._is_adv_mode
            and psname.sec == "SI"
            and not psname.dev.startswith("FC")
        ):
    ...
    def _verify_ps(self, pwrsupplies):
        ...
        if self._ps_failed:
            text = (
                "Verify power state and interlocks"
