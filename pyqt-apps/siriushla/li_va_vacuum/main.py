...
from .functions import buildBasicGroup, buildIPSInfo, buildIdName, buildLed, buildVacPv, getGroupTitle, \
    getLayoutWidget, getVacPosition, getVgcLed, setWindowBtn, showLegend, showUnitView
...
class VacuumMain(QWidget):
    ...
    def showLists(self, cat):
        ...
        if cat == "Vacuum":
            self.showVgcList(config)
        else:      
            self.showIPSList(config)
    ...
    def getGroupWidgets(self, config, item, cat, orient):
        ...
        if(cat == "Valve"):
            ...
            lay, widget = buildBasicGroup(
                self, cat, item, orient)
            buildIPSInfo(
                pv_name, lay, orient)
        else:
            lay, widget = buildBasicGroup(
