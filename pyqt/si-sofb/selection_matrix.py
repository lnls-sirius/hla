...
class SelectionMatrix(QVBoxLayout):
    ...
    SUBSECTIONS = { 'BPMX': ('M1', 'M2', 'C1-1', 'C1-2', 'C2', 'C3-1', 'C3-2', 'C4'),
                    'BPMY': ('M1', 'M2', 'C1-1', 'C1-2', 'C2', 'C3-1', 'C3-2', 'C4'),
                    'CV'  : ('M1', 'M2', 'C1', 'C2-1', 'C2-2', 'C3-1', 'C3-2', 'C4'),
                    'CH'  : ('M1', 'M2', 'C1', 'C2', 'C3','C4'),   }
    INDICES_LENGTH = {'BPMX':NR_BPMs,   'BPMY':NR_BPMs,   'CH':NR_CHs,   'CV':NR_CVs}

    def __init__(self,parent,dev):
        super().__init__(parent)
        self.dev = dev
        # parent.setStyleSheet('QLabel {font: 16pt "Sans Serif";\nfont-weight: bold;}')
        self._setupUi()
    ...
    def _setupUi(self):
        ...
        for i,sec in enumerate(secs):
            wid2 = self._make_line(sec, subsecs, indices[i*len_:(i+1)*len_], False)
            self.addWidget(wid2)
